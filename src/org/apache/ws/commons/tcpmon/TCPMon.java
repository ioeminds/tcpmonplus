	...
	public static void main(String[] args) {
		try {
			...
			if (args.length == 3) {
				...
				ue.setListenPort(p1);
				ue.setTargetHost(args[1]);
				ue.setTargetPort(p2);
				new TCPMon(p1, args[1], p2);
				LastEntryHelper.writeEntry(ue);
				...
				UserEntry ue = LastEntryHelper.getLastEntry();
				if (ue != null) {
					new TCPMon(ue.getListenPort(), ue.getTargetHost(), ue.getTargetPort());
				} else {
					new TCPMon(0, null, 0);
				}
			}
